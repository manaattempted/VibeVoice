"""
VibeVoice Multi-GPU Inference from File

เวอร์ชันปรับปรุงสำหรับเครื่องที่มี NVIDIA GPU หลายใบ (เช่น 4x RTX 3060 Ti)

ความแตกต่างจาก inference_from_file.py:
- ใช้ device_map="auto" แทน "cuda" เพื่อกระจายโมเดลไปทุก GPU (model parallelism)
- รองรับโมเดล 7B (~18GB) บนเครื่อง multi-GPU ที่ VRAM รวม >= 18GB
...
def parse_args():
    parser = argparse.ArgumentParser(description="VibeVoice Multi-GPU Inference")
    parser.add_argument(
        "--model_path",
        type=str,
...
def main():
    ...
    # Show GPU info for multi-GPU setup
    if args.device == "cuda" and torch.cuda.is_available():
        ...
        if total_vram < 18:
            print("  ⚠️ WARNING: Total VRAM < 18GB. The 7B model may not fit!")
    ...
    # Print summary
    print("\n" + "="*50)
    print("GENERATION SUMMARY (Multi-GPU)")
    print("="*50)
